#!/usr/bin/env bash
# TLS + mTLS smoke for many_faces_mailer: generates a demo CA + server + client PEM chain,
# starts docker-compose.tls-smoke.yml, grpcurl grpc.health.v1.Health/Check, then optional .NET channel test.
#
# Prerequisites: bash, openssl, docker compose v2, grpcurl, dotnet 10 SDK when RUN_DOTNET_TLS_SMOKE=1 (default).
#
# Usage (from monorepo root):
#   chmod +x many_faces_mailer/scripts/smoke-grpc-tls.sh
#   many_faces_mailer/scripts/smoke-grpc-tls.sh
#
# Environment:
#   RUN_DOTNET_TLS_SMOKE=0 — skip dotnet test (grpcurl only).
#   MAILER_DIR — override path to many_faces_mailer (default: script parent).
#   MAILER_TLS_SMOKE_PROJECT_NAME — docker compose project name (default: mf-mailer-tls-smoke).

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
MAILER_DIR="${MAILER_DIR:-$(cd "$SCRIPT_DIR/.." && pwd)}"
COMPOSE_FILE="$MAILER_DIR/docker-compose.tls-smoke.yml"
PROJECT_NAME="${MAILER_TLS_SMOKE_PROJECT_NAME:-mf-mailer-tls-smoke}"
RUN_DOTNET_TLS_SMOKE="${RUN_DOTNET_TLS_SMOKE:-1}"

if ! command -v openssl >/dev/null 2>&1; then
  echo "openssl is required" >&2
  exit 1
fi
if ! command -v docker >/dev/null 2>&1; then
  echo "docker is required" >&2
  exit 1
fi
if ! docker compose version >/dev/null 2>&1; then
  echo "docker compose v2 is required" >&2
  exit 1
fi
if ! command -v grpcurl >/dev/null 2>&1; then
  echo "grpcurl is required" >&2
  exit 1
fi

CERT_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mf-mailer-tls-smoke.XXXXXX")"
export MAILER_TLS_SMOKE_CERT_DIR="$(cd "$CERT_DIR" && pwd)"

cleanup() {
  if [[ -f "$COMPOSE_FILE" ]]; then
    (cd "$MAILER_DIR" && docker compose -f "$COMPOSE_FILE" -p "$PROJECT_NAME" down -v 2>/dev/null) || true
  fi
  rm -rf "$CERT_DIR" 2>/dev/null || true
}
trap cleanup EXIT

echo "== Generating demo CA + server + client PEMs in $MAILER_TLS_SMOKE_CERT_DIR"
openssl genrsa -out "$CERT_DIR/ca.key" 2048
openssl req -x509 -new -nodes -key "$CERT_DIR/ca.key" -sha256 -days 1 \
  -subj "/CN=Many Faces Mailer TLS Smoke CA" -out "$CERT_DIR/ca.crt"

openssl genrsa -out "$CERT_DIR/server.key" 2048
openssl req -new -key "$CERT_DIR/server.key" -out "$CERT_DIR/server.csr" \
  -subj "/CN=localhost"
openssl x509 -req -in "$CERT_DIR/server.csr" -CA "$CERT_DIR/ca.crt" -CAkey "$CERT_DIR/ca.key" -CAcreateserial \
  -out "$CERT_DIR/server.crt" -days 1 -sha256 \
  -extfile <(printf '%s\n' 'subjectAltName=DNS:localhost,IP:127.0.0.1')

openssl genrsa -out "$CERT_DIR/client.key" 2048
openssl req -new -key "$CERT_DIR/client.key" -out "$CERT_DIR/client.csr" \
  -subj "/CN=many-faces-mailer-tls-smoke-client"
openssl x509 -req -in "$CERT_DIR/client.csr" -CA "$CERT_DIR/ca.crt" -CAkey "$CERT_DIR/ca.key" -CAcreateserial \
  -out "$CERT_DIR/client.crt" -days 1 -sha256

chmod 755 "$CERT_DIR"
chmod a+r "$CERT_DIR"/*.crt "$CERT_DIR/server.key" 2>/dev/null || true
chmod 600 "$CERT_DIR/client.key"

echo "== Starting mailer-worker (TLS + mTLS)"
(cd "$MAILER_DIR" && docker compose -f "$COMPOSE_FILE" -p "$PROJECT_NAME" up -d --build)

sleep 2

GRPC_PROTO_ROOT="$MAILER_DIR/scripts/grpcurl-protos"

grpcurl_health_check() {
  grpcurl \
    -import-path "$GRPC_PROTO_ROOT" \
    -proto grpc/health/v1/health.proto \
    -servername localhost \
    -cacert "$CERT_DIR/ca.crt" \
    -cert "$CERT_DIR/client.crt" \
    -key "$CERT_DIR/client.key" \
    -d '{"service":""}' \
    127.0.0.1:59216 \
    grpc.health.v1.Health/Check
}

echo "== Waiting for grpc.health Check (grpcurl, TLS + mTLS; no server reflection)"
GRPC_OK=0
for _ in $(seq 1 60); do
  if grpcurl_health_check >/dev/null 2>&1; then
    echo "   grpcurl Health/Check succeeded"
    GRPC_OK=1
    break
  fi
  sleep 2
done
if [[ "$GRPC_OK" != "1" ]]; then
  echo "grpcurl Health/Check did not succeed in time" >&2
  (cd "$MAILER_DIR" && docker compose -f "$COMPOSE_FILE" -p "$PROJECT_NAME" logs --tail 120 mailer-worker) >&2 || true
  grpcurl_health_check >&2 || true
  exit 1
fi

if [[ "$RUN_DOTNET_TLS_SMOKE" == "1" ]]; then
  if ! command -v dotnet >/dev/null 2>&1; then
    echo "dotnet not on PATH; set RUN_DOTNET_TLS_SMOKE=0 to skip .NET smoke" >&2
    exit 1
  fi
  ROOT="$(cd "$MAILER_DIR/.." && pwd)"
  echo "== .NET GrpcChannel + grpc.health (monorepo: $ROOT)"
  export MAILER_TLS_SMOKE=1
  export MAILER_TLS_SMOKE_GRPC_PORT=59216
  export MAILER_TLS_SMOKE_CA="$CERT_DIR/ca.crt"
  export MAILER_TLS_SMOKE_CLIENT_CERT="$CERT_DIR/client.crt"
  export MAILER_TLS_SMOKE_CLIENT_KEY="$CERT_DIR/client.key"
  (cd "$ROOT/many_faces_backend" && dotnet test BeDemo.Api.Tests/BeDemo.Api.Tests.csproj -c Release \
    --filter "FullyQualifiedName~MailerWorkerTlsEndToEndSmokeTests" -v minimal --nologo)
fi

echo "== Mailer TLS smoke completed successfully"
